...
async function analyzeAndOptimize(imageUrl, editPrompt) {
    const env = loadEnv();
    ...
    if (!visionApiKey || !visionApiUrl) {
        throw new Error("Vision API key and URL are required for image analysis. " +
...
export async function repaintImage(params) {
    ...
    const response = await client.generateImageToImage(params.image_url, optimizedPrompt, { size, model });
    return {
        url: response.url,
